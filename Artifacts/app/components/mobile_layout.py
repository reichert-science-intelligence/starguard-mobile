...
def mobile_card(title: str, *content, id: str | None = None, header_color: str | None = None):
    """
...
def mobile_input_group(
    label: str, input_widget, help_text: str | None = None, required: bool = False
):
    ...
    return ui.div(
        ui.tags.label(
            label_text,
            style="font-weight: 600; display: block; margin-bottom: 0.5rem; color: #333;",
        ),
        input_widget,
        ui.tags.small(help_text, class_="text-muted", style="display: block; margin-top: 0.25rem;")
        if help_text
        else None,
        class_="form-group",
...
def mobile_button(text: str, id: str, style: str = "primary", icon: str | None = None):
    """Create touch-optimized button."""
    button_text = f"{icon} {text}" if icon else text
    return ui.input_action_button(
        id, button_text, class_=f"btn btn-{style} btn-block", style="width: 100%;"
    )
...
def metric_box(
    ...
    return ui.div(
        ui.div(
            ui.tags.div(
                label,
                style=f"font-size: 0.875rem; color: {label_color}; font-weight: 700; margin-bottom: 0.25rem;",
            ),
            ui.tags.div(
                value,
                class_="metric-value",
            ...
            ui.tags.div(
                subtitle, style=f"font-size: 0.75rem; color: {subtitle_color}; margin-top: 0.25rem;"
            )
            if subtitle
            else None,
        ),
...
def alert_box(message: str, type: str = "info", dismissible: bool = False):
    ...
    Returns:
        Formatted alert div
    """
    colors = {"success": "#d4edda", "info": "#cfe2ff", "warning": "#fff3cd", "danger": "#f8d7da"}
    text_colors = {
        "success": "#0f5132",
        "info": "#084298",
        "warning": "#664d03",
    ...
    return ui.div(
        ui.div(
            ui.tags.span(
                icon, style=f"font-size: 1.25rem; margin-right: 0.75rem; color: {text_color};"
            ),
    ...
    if text:
        return ui.div(
            ui.div(
                ui.tags.span(
                    text,
                    style="background: white; padding: 0 1rem; color: #999; font-size: 0.875rem; font-weight: 600;",
                ),
...
def progress_bar(
    value: float, max_value: float = 100.0, label: str | None = None, color: str = "#7c3aed"
):
